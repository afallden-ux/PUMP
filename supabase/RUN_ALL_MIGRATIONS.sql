-- =============================================================================
-- PUMP — RUN THIS ENTIRE FILE ONCE (fresh Supabase project)
-- Supabase Dashboard → SQL Editor → New query → Paste all → Run
-- =============================================================================

-- ##############################################################################
-- 001 — Core schema (profiles, workout_logs, leaderboard, avatars)
-- ##############################################################################
...
create table public.profiles (
  ...
  current_pump_score integer not null default 0,
...
create policy "Workout logs viewable by authenticated users"
  on public.workout_logs for select to authenticated using (true);
...
-- ##############################################################################
-- 003 — Grades, bonuses, comments, kudos
-- ##############################################################################
...
create or replace function public.calc_pump_points(
  p_duration_minutes integer, p_intensity_level integer,
  p_is_moonboard boolean default false, p_is_outdoors boolean default false,
...
create or replace function public.enforce_workout_points() returns trigger language plpgsql as $$
begin
  new.total_points := public.calc_pump_points(
    new.duration_minutes, new.intensity_level,
    coalesce(new.is_moonboard, false), coalesce(new.is_outdoors, false), new.hardest_grade
  );
  return new;
end; $$;
...
create policy "Comments viewable by authenticated users" on public.session_comments for select to authenticated using (true);
create policy "Users can insert comments" on public.session_comments for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can delete own comments" on public.session_comments for delete to authenticated using (auth.uid() = user_id);
create policy "Kudos viewable by authenticated users" on public.session_kudos for select to authenticated using (true);
create policy "Users can give kudos" on public.session_kudos for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can remove own kudos" on public.session_kudos for delete to authenticated using (auth.uid() = user_id);
...
create or replace function public.create_crew(p_name text) returns json language plpgsql security definer set search_path = public as $$
declare v_crew public.crews%rowtype; v_code text; v_attempts int := 0;
begin
  ...
  insert into public.crews (name, invite_code, created_by) values (trim(p_name), v_code, auth.uid()) returning * into v_crew;
  insert into public.crew_members (crew_id, user_id, role) values (v_crew.id, auth.uid(), 'owner');
  return json_build_object('id', v_crew.id, 'name', v_crew.name, 'invite_code', v_crew.invite_code, 'role', 'owner');
end; $$;

create or replace function public.join_crew_by_code(p_code text) returns json language plpgsql security definer set search_path = public as $$
...
begin
  ...
  insert into public.crew_members (crew_id, user_id, role) values (v_crew.id, auth.uid(), 'member');
  return json_build_object('id', v_crew.id, 'name', v_crew.name, 'invite_code', v_crew.invite_code, 'role', 'member');
end; $$;

create or replace function public.leave_crew() returns void language plpgsql security definer set search_path = public as $$
declare v_crew_id uuid; v_role text; v_member_count int;
begin
  if auth.uid() is null then raise exception 'Not authenticated'; end if;
...
create or replace function public.delete_crew() returns void language plpgsql security definer set search_path = public as $$
declare v_crew_id uuid;
begin
  select crew_id into v_crew_id from public.crew_members where user_id = auth.uid() and role = 'owner';
...
create or replace function public.regenerate_invite_code() returns text language plpgsql security definer set search_path = public as $$
declare v_crew_id uuid; v_code text; v_attempts int := 0;
begin
  select crew_id into v_crew_id from public.crew_members where user_id = auth.uid() and role = 'owner';
  if v_crew_id is null then raise exception 'Only the crew owner can regenerate the invite code'; end if;
  loop
    v_code := public.generate_invite_code();
    exit when not exists (select 1 from public.crews where invite_code = v_code and id <> v_crew_id);
    v_attempts := v_attempts + 1;
    if v_attempts > 20 then raise exception 'Could not generate invite code'; end if;
  end loop;
...
create policy "Crews visible to members" on public.crews for select to authenticated
  using (exists (select 1 from public.crew_members cm where cm.crew_id = crews.id and cm.user_id = auth.uid()));
...
-- ##############################################################################
-- 005 — Session types + crew battles
-- ##############################################################################
...
begin
  ...
  if p_session_type = 'hangboard' then return base + 15;
  elsif p_session_type = 'gym' then return greatest(5, round(base * 0.75)::integer);
...
begin
  new.total_points := public.calc_pump_points(
    new.duration_minutes, new.intensity_level, coalesce(new.session_type, 'climbing'),
...
create or replace function public.crew_battle_points(p_crew_id uuid, p_starts_at timestamptz, p_ends_at timestamptz)
returns integer language sql stable as $$
  select coalesce(sum(w.total_points), 0)::integer from public.workout_logs w
  inner join public.crew_members cm on cm.user_id = w.user_id
  where cm.crew_id = p_crew_id and w.created_at >= p_starts_at and w.created_at <= p_ends_at;
$$;

create or replace function public.compute_battle_scores(p_battle_id uuid) returns json
language plpgsql stable security definer set search_path = public as $$
declare b public.crew_battles%rowtype; challenger_pts int; opponent_pts int;
begin
  ...
  return json_build_object('challenger_points', challenger_pts, 'opponent_points', opponent_pts, 'ends_at', b.ends_at);
end; $$;

create or replace function public.challenge_crew_battle(p_opponent_code text, p_duration_days integer default 7)
returns json language plpgsql security definer set search_path = public as $$
declare v_my_crew_id uuid; v_opponent_crew public.crews%rowtype; v_battle public.crew_battles%rowtype;
begin
  if auth.uid() is null then raise exception 'Not authenticated'; end if;
  select cm.crew_id into v_my_crew_id from public.crew_members cm where cm.user_id = auth.uid() and cm.role = 'owner';
  ...
  if exists (select 1 from public.crew_battles where status in ('pending', 'active') and (
    (challenger_crew_id = v_my_crew_id and opponent_crew_id = v_opponent_crew.id)
    or (challenger_crew_id = v_opponent_crew.id and opponent_crew_id = v_my_crew_id))) then
    raise exception 'A battle with this crew is already pending or active';
  end if;
  insert into public.crew_battles (challenger_crew_id, opponent_crew_id, status, duration_days, created_by)
  values (v_my_crew_id, v_opponent_crew.id, 'pending', coalesce(p_duration_days, 7), auth.uid()) returning * into v_battle;
  return json_build_object('id', v_battle.id, 'status', v_battle.status, 'opponent_name', v_opponent_crew.name, 'duration_days', v_battle.duration_days);
end; $$;

create or replace function public.accept_crew_battle(p_battle_id uuid) returns json
...
begin
  ...
  if not exists (select 1 from public.crew_members where crew_id = v_battle.opponent_crew_id and user_id = auth.uid() and role = 'owner') then
    raise exception 'Only the challenged crew owner can accept';
  end if;
  update public.crew_battles set status = 'active', starts_at = now(),
    ends_at = now() + (v_battle.duration_days || ' days')::interval where id = p_battle_id returning * into v_battle;
  return json_build_object('id', v_battle.id, 'status', v_battle.status, 'starts_at', v_battle.starts_at, 'ends_at', v_battle.ends_at);
end; $$;

create or replace function public.decline_crew_battle(p_battle_id uuid) returns void
language plpgsql security definer set search_path = public as $$
declare v_battle public.crew_battles%rowtype;
begin
  select * into v_battle from public.crew_battles where id = p_battle_id;
  if not found then raise exception 'Battle not found'; end if;
  if not exists (select 1 from public.crew_members cm where cm.user_id = auth.uid() and cm.role = 'owner'
    and (cm.crew_id = v_battle.opponent_crew_id or cm.crew_id = v_battle.challenger_crew_id)) then
...
create or replace function public.finalize_expired_battles() returns void
language plpgsql security definer set search_path = public as $$
declare b record; c_pts int; o_pts int;
begin
  for b in select * from public.crew_battles where status = 'active' and ends_at < now() loop
    ...
    update public.crew_battles set status = 'completed', winner_crew_id = case
      when c_pts > o_pts then b.challenger_crew_id when o_pts > c_pts then b.opponent_crew_id else null end where id = b.id;
  end loop;
end; $$;

alter table public.crew_battles enable row level security;

create policy "Battles visible to participating crews" on public.crew_battles for select to authenticated
  using (
...
-- =============================================================================
-- DONE. Next steps in Supabase Dashboard (not SQL):
-- 1. Authentication → Providers → enable Email
-- 2. Authentication → URL configuration → your Vercel/domain URLs
-- 3. Database → Replication → enable Realtime on:
--    workout_logs, session_comments, session_kudos, crew_battles
-- =============================================================================
