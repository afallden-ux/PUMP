...
create policy "Crew banners are publicly accessible"
  on storage.objects for select using (bucket_id = 'crew-banners');
...
returns table (
  id uuid, name text, location text, banner_url text,
  member_count bigint, created_at timestamptz
)
language sql stable security definer set search_path = public
as $$
  select c.id, c.name, c.location, c.banner_url,
    (select count(*)::bigint from public.crew_members cm where cm.crew_id = c.id),
    c.created_at
  from public.crews c order by c.name asc;
$$;

create or replace function public.get_public_crew_detail(p_crew_id uuid)
returns json language plpgsql stable security definer set search_path = public as $$
declare v_crew public.crews%rowtype; v_members json; v_count int;
begin
  ...
  select coalesce(json_agg(json_build_object(
    ...
    'title', p.title, 'home_crag', p.home_crag,
    'current_pump_score', p.current_pump_score, 'last_logged_at', p.last_logged_at,
    'role', cm.role
  ) order by cm.role desc, p.username asc), '[]'::json), count(*)::int
  into v_members, v_count
  from public.crew_members cm inner join public.profiles p on p.id = cm.user_id
  where cm.crew_id = p_crew_id;
  return json_build_object(
    'id', v_crew.id, 'name', v_crew.name, 'location', v_crew.location,
    'banner_url', v_crew.banner_url, 'created_at', v_crew.created_at,
    'member_count', v_count, 'members', v_members
  );
end; $$;
...
begin
  ...
  if v_crew_id is null then raise exception 'Only the crew owner can set the crew banner'; end if;
  update public.crews set banner_url = nullif(trim(p_url), '') where id = v_crew_id;
end; $$;
